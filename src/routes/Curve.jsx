...
const Curve = () => {
  ...
  const { curveSupply, curveReserves, curveStep, allBondSteps, debouncedRefresh } = useCurveState(
    bondingCurveAddress,
    { isActive, pollMs: 12000 }
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Raffle ticket token bonding curve overview</p>
        </div>
        <div className="text-right text-sm">
          <div>
            Current / Max Supply: <span className="font-mono">{header.current}</span> / <span className="font-mono">{header.max}</span>
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <BuySellWidget bondingCurveAddress={bondingCurveAddress} onTxSuccess={() => debouncedRefresh(500)} />
          </CardContent>
        </Card>
      </div>
