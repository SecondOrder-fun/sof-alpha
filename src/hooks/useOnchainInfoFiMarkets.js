...
import { listSeasonWinnerMarkets, subscribeMarketCreated } from '@/services/onchainInfoFi'
 ...
 */
export function useOnchainInfoFiMarkets(seasonId, networkKey = 'LOCAL') {
  const [markets, setMarkets] = useState([])
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState(null)

  const sid = useMemo(() => (seasonId == null ? null : Number(seasonId)), [seasonId])
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null)
      try {
        const data = await listSeasonWinnerMarkets({ seasonId: sid, networkKey })
    ...
    const unsub = subscribeMarketCreated({ networkKey, onEvent: (log) => {
      try {
