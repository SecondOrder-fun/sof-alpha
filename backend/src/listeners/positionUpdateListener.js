 ...
 * @param {string} infoFiFactoryAddress - InfoFi factory contract address
...
export async function startPositionUpdateListener(
  ...
  if (!bondingCurveAddress || !raffleAddress || !raffleTokenAddress) {
    throw new Error('bondingCurveAddress, raffleAddress, and raffleTokenAddress are required');
  }
  ...
  if (!infoFiFactoryAddress) {
    throw new Error('infoFiFactoryAddress is required');
  }
  ...
  try {
    ...
    maxSupply = typeof maxSupplyBigInt === 'bigint' ? Number(maxSupplyBigInt) : maxSupplyBigInt;
    ...
    onLogs: async (logs) => {
      for (const log of logs) {
        ...
        try {
          ...
          const oldTicketsNum = typeof oldTickets === 'bigint' ? Number(oldTickets) : oldTickets;
          const newTicketsNum = typeof newTickets === 'bigint' ? Number(newTickets) : newTickets;
          const totalTicketsNum = typeof totalTickets === 'bigint' ? Number(totalTickets) : totalTickets;
          ...
          logger.debug(`   Found ${participants.length} participants in season ${seasonIdNum}`);

          // Step 2: Fetch ticket count for each participant
          logger.debug(`   Fetching positions for ${participants.length} players...`);
          const playerPositions = await Promise.all(
            participants.map(async (addr) => {
              ...
              if (typeof result === 'bigint') {
                ...
                logger.debug(`   Result for ${addr} is object with keys: ${Object.keys(result).join(', ')}`);
                
                // Try common property names
                ticketCount = result.ticketCount || result.tickets || result.amount || result[0];
          ...
          logger.debug(`   Fetched positions for all ${playerPositions.length} players`);
            ...
            try {
              ...
              if (fpmmAddress) {
                ...
                if (result.success) {
                  ...
                  logger.debug(`   ✅ Oracle updated for ${playerAddr}: ${newBps} bps (${result.hash})`);
                } else {
                  logger.warn(`   ⚠️  Oracle update failed for ${playerAddr}: ${result.error}`);
                }
              }
            } catch (oracleError) {
              logger.warn(`   ⚠️  Error updating oracle for ${playerAddr}: ${oracleError.message}`);
            }
          }

          // Step 5: Check if player crossed 1% threshold and trigger market creation
          const oldProbabilityBps = oldTicketsNum > 0 ? Math.round((oldTicketsNum * 10000) / totalTicketsNum) : 0;
          const newProbabilityBps = Math.round((newTicketsNum * 10000) / totalTicketsNum);
          const thresholdBps = 100; // 1% = 100 basis points

          let marketCreationTriggered = false;
          if (oldProbabilityBps < thresholdBps && newProbabilityBps >= thresholdBps) {
          ...
          logger.info(
            ...
            `Market creation: ${marketCreationTriggered ? '✅ Triggered' : '⏭️  Not triggered'}`
          );
  ...
  logger.info(`🎧 Listening for PositionUpdate events on ${bondingCurveAddress}`);
  return unwatch;
}
