...
const BACKEND_ROLE = '0x' + Buffer.from('BACKEND_ROLE').toString('hex').padStart(64, '0');
const BACKEND_ROLE_HASH = '0x3e3a5e0a5b0e8e6c8f5c5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e5e'; // keccak256("BACKEND_ROLE")
...
async function main() {
  ...
  const backendPrivateKey = process.env.BACKEND_WALLET_PRIVATE_KEY || process.env.PRIVATE_KEY;
  ...
  try {
    ...
    if (receipt.status === 'success') {
      ...
      if (hasRoleAfter) {
        ...
        console.log('   Backend can now call onPositionUpdate() to create InfoFi markets\n');
      } else {
        console.error('❌ ERROR: Role grant failed - verification check failed');
        process.exit(1);
      }
    } else {
