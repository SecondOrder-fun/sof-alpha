...
const PLAYER_ADDRESS =
  process.env.PLAYER_ADDRESS || "0xf39Fd6e51aad88F6F4ce6ab8827279cffFb92266";
...
async function diagnose() {
  ...
  try {
    ...
    console.log(
      `   ${factoryMarketAddress === "0x0000000000000000000000000000000000000000" ? "❌ Zero address" : "✅ Valid address"}\n`,
    );
    ...
    console.log(
      `   ${fpmmMarketAddress === "0x0000000000000000000000000000000000000000" ? "❌ Zero address" : "✅ Valid address"}\n`,
    );
    ...
    const [totalTickets, status] = await publicClient.readContract({
      address: RAFFLE_ADDRESS,
      abi: raffleAbi,
    ...
    if (totalTickets > 0n) {
      ...
      console.log(
        `   Player probability: ${probabilityBps.toString()} bps (${Number(probabilityBps) / 100}%)`,
      );
      ...
      console.log(
        `   Market creation threshold: ${threshold.toString()} bps (${Number(threshold) / 100}%)`,
      );
      console.log(
        `   ${probabilityBps >= threshold ? "✅ Above threshold" : "❌ Below threshold"}\n`,
      );
    } else {
    ...
    console.log(
      `   Treasury SOF balance: ${formatUnits(treasuryBalance, 18)} SOF`,
    );
    ...
    console.log(
      `   Required initial liquidity: ${formatUnits(initialLiquidity, 18)} SOF`,
    );
    console.log(
      `   ${treasuryBalance >= initialLiquidity ? "✅ Sufficient balance" : "❌ Insufficient balance"}\n`,
    );
    ...
    if (!marketCreated) {
      ...
      if (playerTickets === 0n) {
        ...
        totalTickets > 0n &&
        (playerTickets * 10000n) / totalTickets < threshold
      ) {
