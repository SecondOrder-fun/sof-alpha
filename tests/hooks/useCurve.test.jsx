...
vi.mock('@/config/contracts', () => ({ getContractAddresses: () => ({ SOF: '0x0000000000000000000000000000000000000aAa' }) }))
vi.mock('@/contracts/abis/SOFBondingCurve.json', () => ({ default: ['buyTokens','sellTokens'] }))
vi.mock('@/contracts/abis/ERC20.json', () => ({ default: { abi: [{ name: 'approve' }] } }))
...
describe('useCurve', () => {
  ...
  it('buyTokens calls curve buyTokens with args', async () => {
    ...
    const { result } = renderHook(() => useCurve('0xCurve...'), { wrapper })
    await act(async () => {
      await result.current.buyTokens.mutateAsync({ tokenAmount: 10n, maxSofAmount: 100n })
    })
    const call = writeContractAsync.mock.calls[0][0]
  ...
  it('sellTokens calls curve sellTokens with args', async () => {
    ...
    const { result } = renderHook(() => useCurve('0xCurve...'), { wrapper })
    await act(async () => {
      await result.current.sellTokens.mutateAsync({ tokenAmount: 5n, minSofAmount: 50n })
    })
    const call = writeContractAsync.mock.calls[0][0]
