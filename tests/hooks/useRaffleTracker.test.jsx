...
vi.mock('@/config/contracts', () => ({
  getContractAddresses: () => ({ RAFFLE_TRACKER: '0x0000000000000000000000000000000000000009' }),
  RAFFLE_TRACKER_ABI: [],
}))
...
describe('useRaffleTracker', () => {
  ...
  it('returns player snapshot when read succeeds', async () => {
    ...
    readContract.mockResolvedValueOnce([100n, 1700000000n, 12345n, 5000n, 200n])
  ...
  it('returns null when address missing (edge)', async () => {
    ...
    expect(result.current.status === 'pending' || result.current.status === 'idle').toBe(true)
  })
