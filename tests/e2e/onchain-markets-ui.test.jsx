...
vi.mock('@/services/onchainInfoFi', () => {
  return {
    ...
    subscribeMarketCreated: vi.fn(({ onEvent }) => {
      ...
      return () => { __marketCreatedHandler = null }
    }),
    ...
    readOraclePrice: vi.fn(async ({ marketId }) => {
      ...
      const active = true
      return { hybridPriceBps, raffleProbabilityBps, marketSentimentBps, lastUpdate, active }
    }),
    subscribeOraclePriceUpdated: vi.fn(({ onEvent }) => {
      __priceUpdatedHandler = onEvent
      return () => { __priceUpdatedHandler = null }
    }),

    // helper used by onchain enumeration output
    computeWinnerMarketId: ({ seasonId, player }) => `0x${String(seasonId)}_${String(player).toLowerCase()}`,
  }
})

// Import hooks under test
const { useOnchainInfoFiMarkets } = await import('@/hooks/useOnchainInfoFiMarkets')
...
function MarketsHarness({ seasonId }) {
  ...
  return (
    <div>
      ...
      <ul>
        {markets.map(m => (
          <li key={m.id} data-testid={`market-${m.id}`}>{m.player}</li>
        ))}
      </ul>
    </div>
  )
...
function triggerMarketCreated({ seasonId, player, marketId }) {
  if (typeof __marketCreatedHandler === 'function') {
    __marketCreatedHandler({ args: { seasonId: BigInt(seasonId), player, marketId } })
  }
}

function triggerPriceUpdated({ marketId, raffleBps, marketBps, hybridBps, timestamp }) {
  if (typeof __priceUpdatedHandler === 'function') {
    __priceUpdatedHandler({ args: { marketId, raffleBps, marketBps, hybridBps, timestamp } })
  }
}
...
describe('On-chain Markets UI (E2E-style)', () => {
  ...
  it('creates a market via MarketCreated and renders live oracle price updates (success case)', async () => {
    ...
    const ui = render(<MarketsHarness seasonId={__seasonId} />)
    ...
    __markets.push({
      ...
      __price: { hybrid: 1111, raffle: 2000, market: 2500, ts: Math.floor(Date.now() / 1000) },
    })
    ...
    await act(async () => {
      triggerPriceUpdated({ marketId, raffleBps: 3000, marketBps: 3200, hybridBps: 3100, timestamp: Date.now() })
    })
  ...
  it('handles oracle read failure gracefully (failure case)', async () => {
    ...
    mod.readOraclePrice.mockImplementationOnce(async () => { throw new Error('oracle down') })

    // Create a single market so the oracle hook mounts against an id
    const player = '0x3333333333333333333333333333333333333333'
    const marketId = `0x${__seasonId}_${player.toLowerCase()}`
    __markets.push({ id: marketId, seasonId: __seasonId, raffle_id: __seasonId, player, market_type: 'WINNER_PREDICTION' })
