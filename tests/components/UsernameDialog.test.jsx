  ...
  useTranslation: () => ({
    t: (key, options) => {
      const translations = {
        ...
        'usernameDialogDescription': 'Choose a unique username to personalize your profile. You can change it later.',
        'username': 'Username',
...
describe('UsernameDialog', () => {
  ...
  it('should render dialog when open', () => {
    ...
    render(
      <UsernameDialog open={true} onOpenChange={onOpenChange} />,
      { wrapper: createWrapper() }
    );

    expect(screen.getByText('Set Username')).toBeInTheDocument();
    expect(screen.getByPlaceholderText('Enter username (3-20 characters)')).toBeInTheDocument();
  });
  ...
  it('should not render dialog when closed', () => {
    ...
    render(
      <UsernameDialog open={false} onOpenChange={onOpenChange} />,
      { wrapper: createWrapper() }
    );
  ...
  it('should show character counter', async () => {
    ...
    const input = screen.getByPlaceholderText('Enter username (3-20 characters)');
  ...
  it('should disable submit button for invalid username', async () => {
    ...
    const submitButton = screen.getByText('Set Username');
