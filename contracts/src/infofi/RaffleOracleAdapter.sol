...
contract RaffleOracleAdapter is AccessControl {
     ...
     */
    function preparePlayerCondition(
        uint256 seasonId,
        address player
    ) external onlyRole(RESOLVER_ROLE) returns (bytes32 conditionId) {
     ...
     *      Only callable by authorized resolvers (InfoFiMarketFactory)
     */
    function resolvePlayerCondition(
        uint256 seasonId,
        address player,
        bool playerWon
